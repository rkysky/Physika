...
#include "Physika_Collision/data_struct/real.hpp"
...
public:
	...
	void push2GPU(bvh_node *r1,bvh_node *r2, unsigned int r1_id,unsigned int r2_id);
};
...
public:
	...
	//rky
	//void self_collide(bvh_node *other,)
...
class bvh {
	...
	void construct(std::vector<mesh*> &ms, int m_id);
...
public:
	...
	//rky
	bvh(std::vector<mesh*> &ms, int m_id);
	...
	//rky
	void refit(std::vector<mesh*> &ms,int m_id);
	void refit(int m_id);

	void push2GPU(int i,int _tri_offset);
